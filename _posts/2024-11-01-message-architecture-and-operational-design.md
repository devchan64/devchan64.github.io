...
> ※ 주석: 본 구조에는 `data_service`에 해당하는 영역이 정의되지 않았습니다. 이는 당시 해당 계층의 역할과 책임이 명확하지 않았기 때문에, 설계 상 의도적으로 비워두고 추후 `data_platform`으로의 통합 가능성을 고려한 전략적 선택이었습니다.
> 
