...
**삭제 가능한 구조, 코드화된 콘텐츠, 자동화된 협업**을 지향합니다.
...
| 원칙 | 설명 |
| --- | --- |
| **삭제 가능한 구조** | 우리는 TailwindCSS와 Notion을 사용하지만, 언제든 교체할 수 있는 구조로 설계한다. |
...
- 변경되는 도구(Tailwind, Notion 등)는 삭제 가능한 구조로 설계되어 교체 시 기술적 리스크가 낮음
...
삭제 가능한 구조와 교체 가능한 기술 스택이 어떤 방식으로 유지될 수 있는지를 정리한다.
...

{
  "blocks": [
    {
      "type": "section",
      "text": { "type": "mrkdwn", "text": "*신규 콘텐츠 PR이 생성되었습니다.*\n<https://github.com/org/repo/pull/123|PR 보기>" }
    },
    {
      "type": "context",
      "elements": [{ "type": "plain_text", "text": "변경자: 마케팅팀 | 태그: 브랜드, 운영", "emoji": true }]
    },
    {
      "type": "actions",
      "elements": [
