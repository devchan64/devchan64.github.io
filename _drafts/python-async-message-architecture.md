...
복잡한 실세계 시스템(로봇, AI 서비스 등)에서는 상태 변화와 응답 지연이 빈번하다.  
전통적인 동기 구조나 상태 중심 시스템은 다음과 같은 문제를 야기한다:

- 통신, 상태, 처리 로직이 얽히며 유지보수성이 저하된다.
- 상태 전이 과정이 코드 곳곳에 분산되어 테스트가 어려워진다.
- 지연 및 재시도 로직이 중복되며 코드 복잡도가 증가한다.

**본 문서의 목표는** Python `asyncio` 기반 비동기 메시지 아키텍처를 설계하여  
이러한 문제를 해결하고, 확장 가능한 시스템 구조를 제시하는 데 있다.
